...
class Query:
    ...
    status: QueryStatus = QueryStatus.UNPROCESSED
    ...
    def __lt__(self, other: "Query") -> bool:
        ...
        return self.openingTimestamp < other.openingTimestamp

    def _blacklist_forever(self) -> None:
        """Blacklist a query forever. Should only be used in cases where it is impossible to query."""
        self.outcomes = None
        self.status = QueryStatus.BLACKLISTED
    ...
    def _check_usefulness(self) -> None:
        """If the query is deemed unhelpful, then blacklist it."""
    ...
    def get_outcome(self, index: int) -> str:
        ...
        if self.outcomes is None:
            raise ValueError(f"Query {self} has an incorrect outcomes list of `None`.")
        try:
...
class QueriesEncoder(json.JSONEncoder):
    """JSON encoder for queries."""

    def default(self, o: Any) -> Any:
        """The default encoder."""
        if isinstance(o, QueryStatus):
            return o.value
...
class QueriesDecoder(json.JSONDecoder):
    """JSON decoder for queries."""

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initialize the Queries JSON decoder."""
        super().__init__(object_hook=self.hook, *args, **kwargs)

    @staticmethod
    def hook(data: Dict[str, Any]) -> Union[Query, Dict[str, Query]]:
        """Perform the custom decoding."""
        # if this is a `Query`
        status_attributes = Query.__annotations__.keys()
...
def serialize_queries(queries: List[Query]) -> Optional[str]:
    """Get the queries serialized."""
    if len(queries) == 0:
        return None
    return json.dumps(queries, cls=QueriesEncoder)